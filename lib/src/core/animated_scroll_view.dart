...
/// {@end-tool}
///
/// By default, [CustomAnimatedList] will automatically pad the limits of the
...
class CustomAnimatedList extends _AnimatedScrollView {
  ...
  ///
  /// If no [CustomAnimatedList] surrounds the context given, then this function will
  ...
  ///    [CustomAnimatedList] ancestor is found.
  static CustomAnimatedListState of(BuildContext context) {
    final CustomAnimatedListState? result = CustomAnimatedList.maybeOf(context);
    assert(() {
      if (result == null) {
  ...
  /// The [CustomAnimatedListState] from the closest instance of [CustomAnimatedList] that encloses the given
  /// context.
  ///
  /// This method is typically used by [CustomAnimatedList] item widgets that insert
  /// or remove items in response to user input.
  ...
  /// See also:
  ///
  ///  * [of], a similar function that will throw if no [CustomAnimatedList] ancestor
  ///    is found.
  static CustomAnimatedListState? maybeOf(BuildContext context) {
    return context.findAncestorStateOfType<CustomAnimatedListState>();
  }
...
/// The [CustomAnimatedListState] for [CustomAnimatedList], a scrolling list container that animates items when they are
/// inserted or removed.
///
...
/// The animation is passed to [CustomAnimatedList.itemBuilder] whenever the item's widget
/// is needed.
///
...
class CustomAnimatedListState extends _AnimatedScrollViewState<CustomAnimatedList> {
  @override
  Widget build(BuildContext context) {
    return _wrap(
      SliverAnimatedList(
...
/// * [CustomAnimatedList], which animates items added and removed from a list instead
///   of a grid.
...
abstract class _AnimatedScrollView extends StatefulWidget {
  ...
  /// been inserted with [CustomAnimatedListState.insertItem] or
  /// [AnimatedGridState.insertItem] and less the total number of items that
  /// have been removed with [CustomAnimatedListState.removeItem] or
  ...
  /// The number of items the [CustomAnimatedList] or [AnimatedGrid] will start with.
  ///
...
abstract class _AnimatedScrollViewState<T extends _AnimatedScrollView> extends State<T> with TickerProviderStateMixin {
  ...
  /// to [AnimatedGrid.itemBuilder] or [CustomAnimatedList.itemBuilder] when the item
  /// is visible.
  ///
  ...
  /// to [AnimatedGrid.itemBuilder] or [CustomAnimatedList.itemBuilder] when the items
  /// are visible.
  ...
  Widget _wrap(Widget sliver, Axis direction) {
    ...
    if (widget.padding == null) {
      ...
      if (mediaQuery != null) {
        ...
        effectivePadding = direction == Axis.vertical ? mediaQueryVerticalPadding : mediaQueryHorizontalPadding;
        // Leave behind the cross axis padding.
        sliver = MediaQuery(
          data: mediaQuery.copyWith(
            padding: direction == Axis.vertical ? mediaQueryHorizontalPadding : mediaQueryVerticalPadding,
          ),
          child: sliver,
        );
      }
...
/// Signature for the builder callback used by [CustomAnimatedList] & [AnimatedGrid] to
...
/// Signature for the builder callback used in [CustomAnimatedListState.removeItem] and
